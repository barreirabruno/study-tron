...
7. After the planning session ends, add its externally captured metrics under:

   * `## Step: planning-metrics`

### 3. Execution

1. Never implement application code or tests without explicit user approval.
2. Execution must start in a fresh Codex session. Do not resume the planning session for execution.
3. After approval, implement only the approved plan.
4. Run the validations required by the approved plan.
5. Stop and report if execution requires an unapproved scope change.
...
5. After the execution session ends, add its externally captured metrics under:

   * `## Step: execution-metrics`

6. Add the comparison between the execution estimates and externally captured execution metrics under:
...
* Use `null` when a required measurement is unavailable and explain why, except for final session token usage, which must use `PENDING_EXTERNAL_CAPTURE` until externally captured.
